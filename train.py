...
print("Importing...", file=sys.stdout, flush=True)
...
parser.add_argument("--var_avg", type=int, default=1)
parser.add_argument("--data_dir", type=str, default="data_preparation/processed/")
...
if __name__ == "__main__":
    ...
    print("Setting up dataloaders...", file=sys.stdout, flush=True)

    loader, test_loader = get_dataloaders(
        cuda=True if device == "cuda" else False, dataloader=args.data_loader
    )
    eval_loader = test_loader
    print("Setting up losses...", file=sys.stdout, flush=True)
    ...
    if device == "cuda":
        print("Using CUDA", file=sys.stdout, flush=True)
        model = model.cuda()
    ...
    wandb_config = {
        ...
        "content_cosine": model.content_cosine,
        "loss_tags": loss_tags,
    ...
    wandb.init(
        ...
        group=group,
        name=(
            f"{loss_tags}-{args.add_name}-{np.random.randint(0, 1000):03d}"
            if len(args.add_name) > 0
            else f"{loss_tags}-{np.random.randint(0, 1000):03d}"
        ),
    ...
    wandb.run.log_code(
        include_fn=lambda path: path.endswith("train.py")
        or path.endswith("split_model.py")
        or path.endswith("utils.py")
    )
    ...
    with tqdm(
        range(BATCHES),
        unit_scale=EFFECTIVE_BATCH_SIZE,
        disable=not USE_TQDM,
        file=sys.stdout,
    ) as pbar:
        for i in pbar:
            current_epoch = int(i / BATCHES * args.epochs)
            model.update_current_epoch(current_epoch)
            model.train()
            optimizer.zero_grad()
            x, loss_dict = model.losses()
            total_loss = sum(
                (model.loss_weights[v] * loss_dict[v] for v in model.used_losses)
            )
            total_loss.backward()
            optimizer.step()
            if OLD_SCHED:
                ...
                with torch.no_grad():
                    (
                        ...
                        eval_loader.get_dataloader(
                            batch_size=model.batch_size, random_sample=False
                        ),
                    )
                    eval_results = get_eval_results(
                        subject_latents, task_latents, subjects, tasks, split="eval"
                    )
                    eval_results.update(
                        {f"eval/{v}": losses[v] for v in list(model.used_losses)}
                    )
                    eval_results.update(
                        {f"train/{v}": loss_dict[v] for v in list(model.used_losses)}
                    )
                    eval_results.update(
                        {
                    ...
                    # my code adding plotting callback of reconstructions
                    loader_tmp = model.loader
                    model.loader = eval_loader
                    model.eval()
                    x = model.get_x_hat({})
                    fig = plot_reconstructions(x, n=5, m=4, model_name=args.module_name)
                    wandb.log({"Running_results/recon": wandb.Image(fig)})
                    plt.close(fig)
                    model.loader = loader_tmp
                    model.train()
                    # my code ends here
    ...
    subject_latents, task_latents, subjects, tasks, runs, losses = get_split_latents(
        model,
        test_loader,
        test_loader.get_dataloader(batch_size=model.batch_size, random_sample=False),
    )
    test_results = get_results(
        ...
        subjects,
        tasks,
        split=test_loader.split,
        off_class_accuracy=args.full_eval,
    )
    wandb.log(test_results)
    if args.conversion_results:
        mse_results = get_full_conversion_results(
            model, test_loader, subject_latents, task_latents, args.conversion_N
        )
        wandb.log(mse_results)
    if args.extra_classifiers:
        test_knn_results = get_results(
            subject_latents,
            task_latents,
            subjects,
            tasks,
    ...
    # for i in range(4):
    #     for j in range(5):
    #         axs[j, i].plot(x["x"][i, j].cpu().detach().numpy(),label="input")
    #         axs[j, i].plot(x["x_hat"][i, j].cpu().detach().numpy(),label="output")
    #         #add target
    #         axs[j, i].plot(x["targets"][i, j].cpu().detach().numpy(),label="target")
    ...
    for i, which in enumerate(["subject", "task", "paradigm"]):
        ...
        if which == "subject":
            ...
            display_labels = [
                f"{test_loader.task_to_label[t]}" for t in test_loader.unique_tasks
            ]
        else:
            cm = (
                task_cm[::2, ::2]
            ...
            display_labels = [
                f'{test_loader.task_to_label[t].split("/")[0]}'
                for t in test_loader.unique_tasks[::2]
            ]
        ...
        disp.plot(
            ...
            cmap="Blues",
            values_format=".2f",
            text_kw={"fontsize": 7} if which == "task" else None,
        )
