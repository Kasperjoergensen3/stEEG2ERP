    ...
    if "t<" in bdf_list:
        time_window = tuple(
            map(
                ...
                bdf_list[bdf_list.find("<") + 1 : bdf_list.find(">")].split("-"),
            )
        )
        if "-" in bdf_list[bdf_list.find(">") + 1 :]:
            event_low, event_high = map(
                int, bdf_list[bdf_list.find(">") + 1 : bdf_list.find("}")].split("-")
            )
            events = range(event_low, event_high + 1)
        else:
            events = map(
                lambda x: x.strip("{}"), bdf_list[bdf_list.find(">") + 1 :].split(";")
            )
    else:
        if "-" in bdf_list:
...
def bin_annotations(annotations, bdf_format, bin_desc):
    ...
    for k, (i, ann) in enumerate(locking_events):
        ...
        if pre_events and i > 0:
            ...
            if pre_ann["description"] in pre_events:
                if pre_time_window:
                    if (
                        onset - pre_ann["onset"] < pre_time_window[0]
                        or onset - pre_ann["onset"] > pre_time_window[1]
                    ):
                        continue
            else:
                continue
        if suc_events and i < len(annotations) - 1:
            ...
            if suc_ann["description"] in suc_events:
                if suc_time_window:
                    if (
                        suc_ann["onset"] - onset < suc_time_window[0]
                        or suc_ann["onset"] - onset > suc_time_window[1]
                    ):
                        continue
            else:
                continue
    ...
    filtered_annotations = [
        ann for i, (_, ann) in enumerate(locking_events) if locking_events_passed[i]
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--full",
        help="Whether to use the fully processed data or not",
        default=False,
    )
    ...
    for para_num, para in enumerate(
        tqdm(["ERN", "LRP", "MMN", "N2pc", "N170", "N400", "P3"])
    ):
        # Folder where the paradigm data is stored
        cur_dir = f"data_preparation/raw/{para} All Data and Scripts/"
        ...
        for subj in tqdm(range(1, 41)):
            ...
            if not full:
                raw_name = (
                    f"{subj}/{subj}_{para}_shifted_ds_reref_ucbip_hpfilt_ica_prep1.set"
                )
            else:
            ...
            if full:
                mne.rename_channels(
                    raw.info, {"(corr) HEOG": "HEOG", "(corr) VEOG": "VEOG"}
                )
                raw.drop_channels(["(uncorr) HEOG", "(uncorr) VEOG"])
            else:
                mne.rename_channels(
                    raw.info, {"(uncorr) HEOG": "HEOG", "(uncorr) VEOG": "VEOG"}
                )
            ...
            if not full:
                if para == "ERN":
                    raw.set_annotations(
                        bin_annotations(
                            raw.annotations,
                            "{t<200-1000>11;12;21;22}.{211;112;221;122}",
                            "Incorrect",
                        )
                        + bin_annotations(
                            raw.annotations,
                            "{t<200-1000>11;12;21;22}.{111;212;121;222}",
                            "Correct",
                        )
                    )
                elif para == "LRP":
                    raw.set_annotations(
                        bin_annotations(
                            raw.annotations, "{t<200-1000>21}.{121}", "Contralateral"
                        )
                        + bin_annotations(
                            raw.annotations, "{t<200-1000>12}.{212}", "Ipsilateral"
                        )
                    )
                elif para == "MMN":
                    raw.set_annotations(
                        bin_annotations(raw.annotations, ".{70}", "Deviants")
                        + bin_annotations(raw.annotations, "{80}.{80}", "Standards")
                    )
                elif para == "N2pc":
                    raw.set_annotations(
                        bin_annotations(
                            raw.annotations,
                            ".{121;122}{t<200-1000>201}",
                            "Contralateral",
                        )
                        + bin_annotations(
                            raw.annotations, ".{211;212}{t<200-1000>201}", "Ipsilateral"
                        )
                    )
                elif para == "N170":
                    raw.set_annotations(
                        bin_annotations(
                            raw.annotations, ".{1-40}{t<200-1000>201}", "Faces"
                        )
                        + bin_annotations(
                            raw.annotations, ".{41-80}{t<200-1000>201}", "Cars"
                        )
                    )
                elif para == "N400":
                    raw.set_annotations(
                        bin_annotations(
                            raw.annotations, ".{221;222}{t<200-1500>201}", "Unrelated"
                        )
                        + bin_annotations(
                            raw.annotations, ".{211;212}{t<200-1500>201}", "Related"
                        )
                    )
                elif para == "P3":
                    raw.set_annotations(
                        bin_annotations(
                            raw.annotations, ".{11;22;33;44;55}{t<200-1000>201}", "Rare"
                        )
                        + bin_annotations(
                            raw.annotations,
                ...
                for n, b in raw.event_id.items():
                    if str(bins[0]) in n[n.find("B") + 1 : n.find("(")]:
                        ...
                        old_bin_to_new[b] = 2
                raw.events[:, -1] = (
                    pd.Series(raw.events[:, -1]).map(old_bin_to_new).to_numpy()
                )
                raw.event_id = merged_event_dict

            # Adding the data to the lists
            data.append(
                raw.get_data(item=[convert_dict_new["1"], convert_dict_new["2"]])
            )
    ...
    torch.save(
        dict(
            ...
            labels=para_label_names,
            data_mean=torch.from_numpy(data_mean),
            data_std=torch.from_numpy(data_std),
        ),
        "data_preparation/processed/" + ("full" if full else "simple") + "_data.pt",
    )
