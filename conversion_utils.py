...
def sample_latents(
    ...
    n=2000,
    specific_task="all",
    specific_subject="all",
):
    if isinstance(specific_subject, int):
        convert_task_latents = task_latents[
            (loader.subjects == specific_subject) & (loader.tasks == target_task)
        ]
    elif specific_subject == "all":
        convert_task_latents = task_latents[loader.tasks == target_task]
    elif specific_subject == "same":
        convert_task_latents = task_latents[
            (loader.subjects == target_subject) & (loader.tasks == target_task)
        ]
        ...
        raise ValueError(
            "specific_subject must be one of [#subject_class_label#, all, same, different]"
        )
    if isinstance(specific_task, int):
        convert_subject_latents = subject_latents[
            (loader.subjects == target_subject) & (loader.tasks == specific_task)
        ]
        ...
        raise ValueError(
            "specific_task must be one of [#task_class_label#, all, same, different]"
        )
...
def reconstruct(model, convert_subject_latents, convert_task_latents, batch_size=2048):
    ...
    convert_subject_latents = torch.unflatten(
        torch.tensor(convert_subject_latents, device="cuda"),
        1,
        (model.latent_dim, model.latent_seqs),
    )
    convert_task_latents = torch.unflatten(
        torch.tensor(convert_task_latents, device="cuda"),
        1,
        (model.latent_dim, model.latent_seqs),
    )
    reconstructions = []
    for i in range(num_batches):
        ...
        x_dict = {
            "s": convert_subject_latents[start_idx:end_idx],
            "t": convert_task_latents[start_idx:end_idx],
        }
...
def get_reconstructed_erps(
    ...
    convert_subject_latents, convert_task_latents = sample_latents(
        ...
        target_task1,
        n=n,
        specific_task=t_spec,
        specific_subject=s_spec,
    )
    ...
    baseline = reconstructions[:, :, baseline1_start:baseline1_end].mean(
        2, keepdim=True
    )
    ...
    reconstructions = reconstruct(model, convert_subject_latents, convert_task_latents)
    reconstructions = reconstructions * loader.data_std + loader.data_mean
    baseline = reconstructions[:, :, baseline2_start:baseline2_end].mean(
        2, keepdim=True
    )
    reconstructions = reconstructions - baseline
...
def get_conversion_results(
    ...
    real_erp1 = loader.data[
        (loader.subjects == target_subject) & (loader.tasks == target_task1)
    ]
    real_erp1 = real_erp1.cuda() * loader.data_std + loader.data_mean
    real_erp1 = real_erp1.mean(0)
    real_erp2 = loader.data[
        (loader.subjects == target_subject) & (loader.tasks == target_task2)
    ]
    real_erp2 = real_erp2.cuda() * loader.data_std + loader.data_mean
    real_erp2 = real_erp2.mean(0)
    recon_erp_ss1, recon_erp_ss2 = get_reconstructed_erps(
        model,
        loader,
        subject_latents,
        task_latents,
    ...
    recon_erp_sd1, recon_erp_sd2 = get_reconstructed_erps(
        model,
        loader,
        subject_latents,
        task_latents,
    ...
    recon_erp_ds1, recon_erp_ds2 = get_reconstructed_erps(
        model,
        loader,
        subject_latents,
        task_latents,
    ...
    recon_erp_dd1, recon_erp_dd2 = get_reconstructed_erps(
        model,
        loader,
        subject_latents,
        task_latents,
    ...
    ss_mse = F.mse_loss(recon_erp_ss1[channel], real_erp1[channel]) + F.mse_loss(
        recon_erp_ss2[channel], real_erp2[channel]
    )
    sd_mse = F.mse_loss(recon_erp_sd1[channel], real_erp1[channel]) + F.mse_loss(
        recon_erp_sd2[channel], real_erp2[channel]
    )
    ds_mse = F.mse_loss(recon_erp_ds1[channel], real_erp1[channel]) + F.mse_loss(
        recon_erp_ds2[channel], real_erp2[channel]
    )
    dd_mse = F.mse_loss(recon_erp_dd1[channel], real_erp1[channel]) + F.mse_loss(
        recon_erp_dd2[channel], real_erp2[channel]
    )
...
def get_full_conversion_results(model, test_loader, subject_latents, task_latents, N):
    ...
    for target_subject in test_loader.unique_subjects:
        for target_task1, target_task2 in test_loader.paradigms:
            ...
            ss_mse, sd_mse, ds_mse, dd_mse = get_conversion_results(
                model,
                test_loader,
                subject_latents,
                task_latents,
    ...
    for target_subject in test_loader.unique_subjects:
        ...
        for target_task1, target_task2 in test_loader.paradigms:
            paradigm_name = test_loader.task_to_label[target_task1].split("/")[0]
            ss_results.append(
                mse_results[f"MSE/test/{paradigm_name}/{target_subject}/ss"]
            )
            sd_results.append(
                mse_results[f"MSE/test/{paradigm_name}/{target_subject}/sd"]
            )
            ds_results.append(
                mse_results[f"MSE/test/{paradigm_name}/{target_subject}/ds"]
            )
            dd_results.append(
                mse_results[f"MSE/test/{paradigm_name}/{target_subject}/dd"]
            )
        mse_results[f"MSE/test/mean/{target_subject}/ss"] = torch.mean(
            torch.stack(ss_results)
        )
        mse_results[f"MSE/test/mean/{target_subject}/sd"] = torch.mean(
            torch.stack(sd_results)
        )
        mse_results[f"MSE/test/mean/{target_subject}/ds"] = torch.mean(
            torch.stack(ds_results)
        )
        mse_results[f"MSE/test/mean/{target_subject}/dd"] = torch.mean(
            torch.stack(dd_results)
        )
    ...
    for target_task1, target_task2 in test_loader.paradigms:
        ...
        mse_results[f"MSE/test/mean/{paradigm_name}/ss"] = torch.mean(
            torch.stack(ss_results)
        )
        mse_results[f"MSE/test/mean/{paradigm_name}/sd"] = torch.mean(
            torch.stack(sd_results)
        )
        mse_results[f"MSE/test/mean/{paradigm_name}/ds"] = torch.mean(
            torch.stack(ds_results)
        )
        mse_results[f"MSE/test/mean/{paradigm_name}/dd"] = torch.mean(
            torch.stack(dd_results)
        )
