...
def plot_latents(fig, ax, latents, loader, which="subject", size=1, cmap=None):
    ...
    cluster_colors = [
        color_palette[x] for x in np.vectorize(idx_trans.get)(which_values)
    ]
    ...
    for i in which_uniques:
        ...
        ax.scatter(
            ...
            y,
            color=c[0],
            s=size if loader.split == "train" else 2 * size,
            label=f"{task_to_label[i]}" if which == "task" else f"S{i}",
            alpha=0.5 if loader.split == "train" else 1.0,
        )
        ax.grid(False)
        ax.set_xticks([])
        ax.set_yticks([])
...
def get_split_latents(model, loader, dataloader):
    ...
    for idxs, xs, ss, ts, rs, tgts in dataloader:  # my addition
        xs = xs.cuda()
        ss = ss.cuda()
        ts = ts.cuda()
        tgts = tgts.cuda()  # my addition
        x_dict = {"x": xs, "S": ss - 1, "T": ts, "tgt": tgts}
...
def get_split_latents_new(model, loader, dataloader, device="cuda", use_tqdm=False):
    model.eval()
    subject_latents = []
    task_latents = []
    subjects = []
    ...
    iterator = dataloader if not use_tqdm else tqdm(dataloader)
    for idxs, xs, ss, ts, rs, tgts in iterator:
        xs = xs.cuda() if device == "cuda" else xs
        ss = ss.cuda() if device == "cuda" else ss
        ts = ts.cuda() if device == "cuda" else ts
        tgts = tgts.cuda() if device == "cuda" else tgts
        x_dict = {"x": xs, "S": ss - 1, "T": ts, "tgt": tgts}
        x_dict, loss_dict = model.losses(x_dict, model.used_losses, loader=loader)
        for l_name, l_val in loss_dict.items():
            losses[l_name].append(l_val)
...
class XGBWrapper:
    ...
    def fit(self, X, y):
        ...
        class_weights = {
            i: min(class_counts.values()) / class_counts[i] for i in class_counts.keys()
        }
...
def balanced_sample(which, sampling_method="under"):
    ...
    for t, idxs in which_idxs.items():
        ...
        flat_idxs.extend(
            np.random.choice(
                idxs, size=count, replace=not sampling_method.lower().startswith("u")
            )
        )
    return flat_idxs
...
def get_results(
    ...
    for train_idx, test_idx in tqdm(subject_splits):
        ...
        subject_metrics["precision"].append(
            precision_score(y_true, y_pred, average="weighted")
        )
        subject_metrics["recall"].append(
            recall_score(y_true, y_pred, average="weighted")
        )
        subject_metrics["f1"].append(f1_score(y_true, y_pred, average="weighted"))
        subject_metrics["balanced_accuracy"].append(
        ...
        for s, pr, re, f1 in zip(
            np.unique(y_true), precision_scores, recall_scores, f1_scores
        ):
        ...
        if off_class_accuracy:
            X_task_train, X_task_test = (
                task_latents[train_idx][flat_idxs],
                task_latents[test_idx],
            )
    ...
    for train_idx, test_idx in tqdm(task_splits):
        ...
        task_metrics["balanced_accuracy"].append(
            balanced_accuracy_score(y_true, y_pred)
        )
        ...
        if off_class_accuracy:
            X_subject_train, X_subject_test = (
                subject_latents[train_idx][flat_idxs],
                subject_latents[test_idx],
            )
    ...
    if off_class_accuracy:
        ...
        test_results[clf + "/" + split + "/task/subject_on_task_score"] = (
            subject_on_task_score
        )
        test_results[clf + "/" + split + "/subject/task_on_subject_score"] = (
            task_on_subject_score
        )
        test_results[clf + "/" + split + "/task/subject_on_task_scores"] = (
            subject_on_task_scores
        )
        test_results[clf + "/" + split + "/subject/task_on_subject_scores"] = (
            task_on_subject_scores
        )
    ...
    test_results[clf + "/" + split + "/task/paradigm_wise_accuracy"] = np.mean(
        np.sum(task_cm_pair_blocks, axis=(1, 2)) / task_cm_pair_true
    )
...
def get_eval_results(
    ...
    X_task_train, X_task_test, tasks_train, tasks_test = train_test_split(
        task_latents,
        tasks,
        stratify=tasks,
        test_size=0.2,
...
def plot_reconstructions(x, n=5, m=4, model_name=""):
    if model_name == "SplitLatentModelWithVariance":
        add_confidence = True
    else:
        add_confidence = False

    fig, axs = plt.subplots(n, m, figsize=(n * 4, m * 3.75))
    for i in range(m):
        for j, c in enumerate([0, 7, 14, 21, 28]):
            if add_confidence:
                axs[j, i].fill_between(
                    range(x["x_hat"].shape[-1]),
                    x["x_hat"][i, c].cpu().detach().numpy()
                    - 2 * np.sqrt(x["x_hat_var"][i, c].cpu().detach().numpy()),
                    x["x_hat"][i, c].cpu().detach().numpy()
                    + 2 * np.sqrt(x["x_hat_var"][i, c].cpu().detach().numpy()),
                    alpha=0.3,
                )
            axs[j, i].plot(x["x"][i, c].cpu().detach().numpy(), label="input")
            axs[j, i].plot(x["x_hat"][i, c].cpu().detach().numpy(), label="output")

            # add target
            axs[j, i].plot(x["tgt"][i, c].cpu().detach().numpy(), label="target")
            axs[j, i].set_title(f"Sample {i}, Channel {c}")
    plt.tight_layout()
    return fig
