    ...
    def __init__(self, channels, kernel_size, num_layers):
        ...
        def get_conv_layer():
            return nn.Sequential(
                ...
                nn.Conv1d(
                    channels, channels, kernel_size, stride=2, bias=False, padding=1
                ),
            )
...
class TransposedConvolutionalDecoder(nn.Module):
    def __init__(self, channels, kernel_size, num_layers):
        ...
        def get_conv_layer():
            return nn.Sequential(
                nn.ConvTranspose1d(
                    channels, channels, kernel_size, stride=2, padding=1, bias=False
                ),
                ConvBlock(channels, channels, channels),
            )
        ...
        def get_transformer_layer():
            return (
                ...
                nn.TransformerEncoderLayer(
                    channels, 1, channels, 0.0, activation="relu", batch_first=True
                ),
            )

        self.transformer_layers = nn.Sequential(
            *[l for _ in range(num_layers) for l in get_transformer_layer()]
        )
...
class BaselineModel(nn.Module):
    def __init__(
        ...
        self.encoder_bottleneck = TransformerBottleneck(
            channels, num_layers, self.latent_seqs
        )
        ...
        self.decoder_bottleneck = TransformerBottleneck(
            channels, num_layers, self.latent_seqs
        )
    ...
    def set_losses(self, losses=[], batch_size=256, loader=None, loss_weights=None):
        ...
        if not self.sub_clf_layer_init and any(
            ["sub_cross" in l for l in self.used_losses]
        ):
            self.sub_clf_layer = nn.Linear(self.latent_seqs * self.latent_dim, 40)
            self.sub_clf_layer_init = True
        if not self.task_clf_layer_init and any(
            ["task_cross" in l for l in self.used_losses]
        ):
            self.task_clf_layer = nn.Linear(self.latent_seqs * self.latent_dim, 14)
            self.task_clf_layer_init = True
    ...
    def reconstruction_loss(self, x, x_hat):
        ...
        elif self.recon_type.lower().startswith("log_mse"):
            return torch.log(F.mse_loss(x, x_hat, reduction="mean"))
        return F.mse_loss(x, x_hat, reduction="mean")
    ...
    def get_x(self, x):
        _, x1, s1, t1, r1, tgt1 = self.loader.sample_batch(
            self.batch_size
        )  # my addition
        x["x"] = x1.cuda() if DEVICE == "cuda" else x1
        x["S"] = torch.tensor(s1 - 1, device=DEVICE)
        x["T"] = torch.tensor(t1, device=DEVICE)
        x["tgt"] = tgt1.cuda() if DEVICE == "cuda" else tgt1
        # x['R'] = r1.cuda()
        return x
    ...
    def get_x_prop(self, x, prop="s"):
        if "x_p_" + prop + "1" not in x or "x_p_" + prop + "2" not in x:
            ...
            _, x1, s1, t1, r1, t1 = self.loader.sample_by_property(prop)
            _, x2, s2, t2, r2, t2 = self.loader.sample_by_property(prop)
            ...
            targets1 = torch.stack(targets1, 0)
            targets2 = torch.stack(targets2, 0)
            ...
            x["x_p_" + prop + "1"] = samples1.cuda() if DEVICE == "cuda" else samples1
            x["x_p_" + prop + "2"] = samples2.cuda() if DEVICE == "cuda" else samples2
            x["tgt_p_" + prop + "1"] = targets1.cuda() if DEVICE == "cuda" else targets1
            x["tgt_p_" + prop + "2"] = targets2.cuda() if DEVICE == "cuda" else targets2
        return x
    ...
    def get_l_prop(self, x, prop="s"):
        ...
        if "l_p_" + prop + "1" not in x or "l_p_" + prop + "2" not in x:
            x["l_p_" + prop + "1"] = self.collapsed_apply(
                x["x_p_" + prop + "1"], self.encode
            )
            x["l_p_" + prop + "2"] = self.collapsed_apply(
                x["x_p_" + prop + "2"], self.encode
            )
        return x
    ...
    def get_x_hat_prop(self, x, prop="s"):
        ...
        if "x_hat_p_" + prop + "1" not in x or "x_hat_p_" + prop + "2" not in x:
            x["x_hat_p_" + prop + "1"] = self.collapsed_apply(
                x["l_p_" + prop + "1"], self.decode
            ).view_as(x["x_p_" + prop + "1"])
            x["x_hat_p_" + prop + "2"] = self.collapsed_apply(
                x["l_p_" + prop + "2"], self.decode
            ).view_as(x["x_p_" + prop + "2"])
        return x
    ...
    def losses(
        self, x=None, which=["recon", "contra", "ortho", "content"], loader=None
    ):
...
class SplitLatentModel(BaselineModel):
    ...
    def __init__(
        ...
        super(SplitLatentModel, self).__init__(
            in_channels,
            channels,
            latent_dim,
            num_layers,
    ...
    def get_s_t(self, x):
        if "x" not in x or "tgt" not in x:  # my addition
            x = self.get_x(x)
    ...
    def get_x_hat(self, x):
        if "s" not in x or "t" not in x or "tgt" not in x:  # my addition
            x = self.get_s_t(x)
    ...
    def get_s_t_prop(self, x, prop):
        if "x_p_" + prop + "1" not in x or "x_p_" + prop + "2" not in x:
            x = self.get_x_prop(x, prop)
        if (
            "s_p_" + prop + "1" not in x
            or "t_p_" + prop + "1" not in x
            or "s_p_" + prop + "2" not in x
            or "t_p_" + prop + "2" not in x
        ):
            s1, t1 = self.collapsed_split_apply(
                x["x_p_" + prop + "1"], self.subject_task_encode
            )
            s2, t2 = self.collapsed_split_apply(
                x["x_p_" + prop + "2"], self.subject_task_encode
            )
            x["s_p_" + prop + "1"] = s1
            x["t_p_" + prop + "1"] = t1
            x["s_p_" + prop + "2"] = s2
    ...
    def get_x_hat_prop(self, x, prop):
        ...
        x["x_hat_p_" + prop + "1"] = self.subject_task_decode(
            x["s_p_" + prop + "1"], x["t_p_" + prop + "1"]
        ).view_as(x["x_p_" + prop + "1"])
        x["x_hat_p_" + prop + "2"] = self.subject_task_decode(
            x["s_p_" + prop + "2"], x["t_p_" + prop + "2"]
        ).view_as(x["x_p_" + prop + "2"])
        return x
    ...
    def get_x_hat_prop_only_permuted(self, x, prop):
        ...
        if "ldetachswitch" in self.recon_type:
            if prop == "s":
                x["x_hat_p_" + prop + "_s1t2"] = self.subject_task_decode(
                    x["s_p_" + prop + "1"].detach(), x["t_p_" + prop + "2"]
                ).view_as(x["x_p_" + prop + "1"])
                x["x_hat_p_" + prop + "_s2t1"] = self.subject_task_decode(
                    x["s_p_" + prop + "2"].detach(), x["t_p_" + prop + "1"]
                ).view_as(x["x_p_" + prop + "2"])
            elif prop == "t":
                x["x_hat_p_" + prop + "_s1t2"] = self.subject_task_decode(
                    x["s_p_" + prop + "1"], x["t_p_" + prop + "2"].detach()
                ).view_as(x["x_p_" + prop + "1"])
                x["x_hat_p_" + prop + "_s2t1"] = self.subject_task_decode(
                    x["s_p_" + prop + "2"], x["t_p_" + prop + "1"].detach()
                ).view_as(x["x_p_" + prop + "2"])
            ...
            x["x_hat_p_" + prop + "_s1t2"] = self.subject_task_decode(
                x["s_p_" + prop + "1"], x["t_p_" + prop + "2"]
            ).view_as(x["x_p_" + prop + "1"])
            x["x_hat_p_" + prop + "_s2t1"] = self.subject_task_decode(
                x["s_p_" + prop + "2"], x["t_p_" + prop + "1"]
            ).view_as(x["x_p_" + prop + "2"])
        return x

    def get_s_t_hat_prop_any(self, x, prop):
        if "x_hat_p_" + prop + "1" in x:
            s_hat_s1t1, t_hat_s1t1 = self.collapsed_split_apply(
                x["x_hat_p_" + prop + "1"], self.subject_task_encode
            )
            x["s_hat_p_" + prop + "1"] = s_hat_s1t1
            x["t_hat_p_" + prop + "1"] = t_hat_s1t1
        if "x_hat_p_" + prop + "2" in x:
            s_hat_s2t2, t_hat_s2t2 = self.collapsed_split_apply(
                x["x_hat_p_" + prop + "2"], self.subject_task_encode
            )
            x["s_hat_p_" + prop + "2"] = s_hat_s2t2
            x["t_hat_p_" + prop + "2"] = t_hat_s2t2
        if "x_hat_p_" + prop + "_s1t2" in x:
            s_hat_s1t2, t_hat_s1t2 = self.collapsed_split_apply(
                x["x_hat_p_" + prop + "_s1t2"], self.subject_task_encode
            )
            x["s_hat_p_" + prop + "_s1t2"] = s_hat_s1t2
            x["t_hat_p_" + prop + "_s1t2"] = t_hat_s1t2
        if "x_hat_p_" + prop + "_s2t1" in x:
            s_hat_s2t1, t_hat_s2t1 = self.collapsed_split_apply(
                x["x_hat_p_" + prop + "_s2t1"], self.subject_task_encode
            )
    ...
    def get_x_hat_hat_restored(self, x, prop):
        ...
        if (
            "s_hat_p_" + prop + "_s2t1" not in x
            or "t_hat_p_" + prop + "_s2t1" not in x
            or "s_hat_p_" + prop + "_s1t2" not in x
            or "t_hat_p_" + prop + "_s1t2" not in x
        ):
            x = self.get_s_t_hat_prop_any(x, prop)
        if "rswitch" in self.recon_type:
            x["x_hat_hat_p_" + prop + "1"] = self.subject_task_decode(
                x["s_hat_p_" + prop + "_s2t1"], x["t_hat_p_" + prop + "_s1t2"]
            ).view_as(x["x_hat_p_" + prop + "_s1t2"])
            x["x_hat_hat_p_" + prop + "2"] = self.subject_task_decode(
                x["s_hat_p_" + prop + "_s1t2"], x["t_hat_p_" + prop + "_s2t1"]
            ).view_as(x["x_hat_p_" + prop + "_s2t1"])
        else:
            x["x_hat_hat_p_" + prop + "2"] = self.subject_task_decode(
                x["s_hat_p_" + prop + "_s2t1"], x["t_hat_p_" + prop + "_s1t2"]
            ).view_as(x["x_hat_p_" + prop + "_s2t1"])
        ...
        for k in which:
            ...
            if space is None:
                if k.startswith("recon"):
                    if "x_hat" not in x or "tgt" not in x:  # my addition
                        x = self.get_x_hat(x)
                    # loss_dict[k] = self.reconstruction_loss(x["x"], x["x_hat"])
                    loss_dict[k] = self.reconstruction_loss(
                        x["tgt"], x["x_hat"]
                    )  # my addition
                    ...
                    loss_dict[k] = self.cosine_distance_loss(
                        x["tgt"], x["x_hat"]
                    )  # my addition
                    ...
                    if self.loader is None:
                        ...
                        if (
                            "x_hat_p_" + prop + "_s1t2" not in x
                            or "x_hat_p_" + prop + "_s2t1" not in x
                        ):
                            x = self.get_x_hat_prop_only_permuted(x, prop)
                        if "lswitch" in self.recon_type.lower():
                            if prop == "s":
                                loss_dict[k + "_1"] = self.reconstruction_loss(
                                    x["x_p_" + prop + "1"],
                                    x["x_hat_p_" + prop + "_s1t2"],
                                )
                                loss_dict[k + "_2"] = self.reconstruction_loss(
                                    x["x_p_" + prop + "2"],
                                    x["x_hat_p_" + prop + "_s2t1"],
                                )
                            elif prop == "t":
                                loss_dict[k + "_1"] = self.reconstruction_loss(
                                    x["x_p_" + prop + "1"],
                    ...
                    if self.loader is None:
                        ...
                        subjects1 = np.random.choice(
                            self.loader.unique_subjects, Q_batch_size, replace=True
                        )
                        subjects2 = np.random.choice(
                            self.loader.unique_subjects, Q_batch_size, replace=True
                        )
                        tasks1 = np.random.choice(
                            self.loader.unique_tasks, Q_batch_size, replace=True
                        )
                        tasks2 = np.random.choice(
                            self.loader.unique_tasks, Q_batch_size, replace=True
                        )
                        all_subjects = np.concatenate(
                            [subjects1, subjects1, subjects2, subjects2]
                        )
                        all_tasks = np.concatenate([tasks1, tasks2, tasks1, tasks2])
                        all_x = self.loader.sample_by_condition(all_subjects, all_tasks)
                        all_x = all_x.cuda() if DEVICE == "cuda" else all_x
                        all_s, all_t = self.subject_task_encode(all_x)
                        s11, t11 = all_s[:Q_batch_size], all_t[:Q_batch_size]
                        s12, t12 = (
                            all_s[Q_batch_size : 2 * Q_batch_size],
                            all_t[Q_batch_size : 2 * Q_batch_size],
                        )
                        s21, t21 = (
                            all_s[2 * Q_batch_size : 3 * Q_batch_size],
                            all_t[2 * Q_batch_size : 3 * Q_batch_size],
                        )
                    ...
                    if self.loader is None:
                        ...
                        prop = self.determine_prop(k)
                        if (
                            "x_hat_hat_p_" + prop + "1" not in x
                            or "x_hat_hat_p_" + prop + "2" not in x
                        ):
                            x = self.get_x_hat_hat_restored(x, prop)
                        # loss_dict[k + "_1"] = self.reconstruction_loss(
                        #     x["x_p_" + prop + "1"], x["x_hat_hat_p_" + prop + "1"]
                        # )
                        # loss_dict[k + "_2"] = self.reconstruction_loss(
                        #     x["x_p_" + prop + "2"], x["x_hat_hat_p_" + prop + "2"]
                        # )
                        loss_dict[k + "_1"] = self.reconstruction_loss(
                            x["tgt_p_" + prop + "1"], x["x_hat_hat_p_" + prop + "1"]
                        )
                        loss_dict[k + "_2"] = self.reconstruction_loss(
                            x["tgt_p_" + prop + "2"], x["x_hat_hat_p_" + prop + "2"]
                        )
                        loss_dict[k] = 0.5 * (loss_dict[k + "_1"] + loss_dict[k + "_2"])
                ...
                if "contra" in k:
                    if self.loader is None:
                        raise ValueError(
                            "Need to pass loader to compute contrastive loss"
                        )
                    else:
                        prop = self.determine_prop(k)
                        if (
                            space + "_p_" + prop + "1" not in x
                            or space + "_p_" + prop + "2" not in x
                        ):
                            x = self.get_s_t_prop(x, prop)
                        contr1, contr2 = self.contrastive_loss(
                            x[space + "_p_" + prop + "1"],
                            x[space + "_p_" + prop + "2"],
                            which=space,
                        )
                    ...
                    if space not in x:
                        x = self.get_s_t(x)
                    loss_dict[k] = self.cross_entropy_loss(
                        x[space], x[space.upper()], space
                    )
                else:
                    self.loader = old_loader
...
class AveragingLinearLayer(nn.Module):
    def __init__(self, time_dim):
        super(AveragingLinearLayer, self).__init__()
        self.time_dim = time_dim
        # Initialize the linear layer to go from `time_dim` features to 1 feature
        self.linear = nn.Linear(time_dim, 1)

    def forward(self, x):
        batch_size, channels, time = x.size()
        # Reshape x to [Batch*Channel, Time] to apply linear layer
        x = x.view(batch_size * channels, time)
        # Apply the linear layer and then reshape to [Batch, Channel]
        x = self.linear(x)
        x = x.view(batch_size, channels)
        return x


class SplitLatentModelWithVariance(SplitLatentModel):
    """
    stEEG2ERP model
    """
    ...
    def __init__(
        ...
        super(SplitLatentModelWithVariance, self).__init__(
            in_channels,
            channels,
            latent_dim,
            num_layers,
        ...
        # identical architecture to the decoder for mean
        self.decoder_var = TransposedConvolutionalDecoder(
            channels, kernel_size, num_layers
        )
        self.decoder_out_var = ConvBlock(channels, channels, in_channels)
        self.decoder_out_var_av = AveragingLinearLayer(256)
        self.current_epoch = 0  # Initialize current epoch
        self.freeze_until = 100  # Freeze variance decoder until this epoch
        self.var_av = var_av

    def update_current_epoch(self, epoch):
        self.current_epoch = epoch
        if epoch <= self.freeze_until:
            self.freeze_variance_decoder()
        else:
            self.unfreeze_variance_decoder(unfreeze=True)

    def freeze_variance_decoder(self, unfreeze=False):
        for param in self.decoder_var.parameters():
            param.requires_grad = unfreeze
        for param in self.decoder_out_var.parameters():
            param.requires_grad = unfreeze

    def subject_task_decode(self, s, t, return_var=False):
        s = s.view(-1, self.latent_dim, self.latent_seqs)
        t = t.view(-1, self.latent_dim, self.latent_seqs)
        x = torch.cat([s, t], dim=1)
        x = self.decoder_in_net(x)
    ...
    def post_decode(self, x):
        ...
        # get mean
        mu = self.decoder(x)
        mu = self.decoder_out(mu)
        # get variance
        if self.current_epoch <= self.freeze_until:
            var = torch.ones_like(mu).to(DEVICE)
        else:
            var = self.decoder_var(x)
            # print('after decoder_var')
            # print(var[0,0,:8])
            # print(var[1,0,:8])
            var = self.decoder_out_var(var)
            # print('after decoder_out_var')
            # print(var[0,0,:8])
            # print(var[1,0,:8])
            if self.var_av:
                var = self.decoder_out_var_av(var)
                var = var.unsqueeze(-1).expand(-1, -1, mu.shape[-1])

            # print('after mean')
            # print(torch.abs(var[0]-var[1]).sum())
            # print(var[0])
            # print(var[1])
            # print(var.shape)

            var = F.softplus(var)  # softplus to ensure positive variance
            # print(var.shape)
            # print(var.shape)
            # repeat variance to match the shape of mu
        return mu, var

    def get_x_hat(self, x):
        if "s" not in x or "t" not in x or "tgt" not in x:  # my addition
            x = self.get_s_t(x)
        x["x_hat"], x["x_hat_var"] = self.subject_task_decode(
            x["s"], x["t"], return_var=True
        )
        return x
    ...
    def get_x_hat_hat_restored(self, x, prop):
        ...
        if (
            ...
            x = self.get_s_t_hat_prop_any(x, prop)
        if "rswitch" in self.recon_type:
            x["x_hat_hat_p_" + prop + "1"], x["x_hat_hat_p_" + prop + "1_var"] = (
                self.subject_task_decode(
                    x["s_hat_p_" + prop + "_s2t1"],
                    x["t_hat_p_" + prop + "_s1t2"],
                    return_var=True,
                )
            )
            x["tgt_p_" + prop + "1"] = x["tgt_p_" + prop + "2"].view_as(
                x["x_hat_hat_p_" + prop + "1"]
            )
            x["x_hat_hat_p_" + prop + "2"], x["x_hat_hat_p_" + prop + "2_var"] = (
                self.subject_task_decode(
                    x["s_hat_p_" + prop + "_s1t2"],
                    x["t_hat_p_" + prop + "_s2t1"],
                    return_var=True,
                )
            )
            x["tgt_p_" + prop + "2"] = x["tgt_p_" + prop + "1"].view_as(
                x["x_hat_hat_p_" + prop + "2"]
            )
    ...
    def reconstruction_loss(self, x, x_hat, x_hat_var):
        return -Normal(x_hat, torch.sqrt(x_hat_var)).log_prob(x).mean()
                ...
                if k.startswith("recon"):
                    ...
                    if self.loader is None:
                        ...
                        loss_dict[k + "_1"] = self.reconstruction_loss(
                            x["tgt_p_" + prop + "1"],
                            x["x_hat_hat_p_" + prop + "1"],
                            x["x_hat_hat_p_" + prop + "1_var"],
                        )
                        loss_dict[k + "_2"] = self.reconstruction_loss(
                            x["tgt_p_" + prop + "2"],
                            x["x_hat_hat_p_" + prop + "2"],
                            x["x_hat_hat_p_" + prop + "2_var"],
                        )
                        loss_dict[k] = 0.5 * (loss_dict[k + "_1"] + loss_dict[k + "_2"])
            else:
                if "contra" in k:
    ...
    def forward(self, x, return_var=False):
        s, t = self.subject_task_encode(x)
        if return_var:
            mu, var = self.subject_task_decode(s, t, return_var=True)
            return mu, var
        else:
            mu = self.subject_task_decode(s, t)
            return mu
